...
const Courses11 = () => {
  ...
  const [matchingCourseNamesData, setMatchingCourseNamesData] = useState('');
  ...
  useEffect(() => {
    ...
    const fetchCourses = async () => {
      ...
      const storedMatchingCourseNamesData = localStorage.getItem('matchingCourseNames');
      if (storedMatchingCourseNamesData) {
        setMatchingCourseNamesData(JSON.parse(storedMatchingCourseNamesData));
      }
    };
  ...
  const clearAndAddToLocalStorage = (data) => {
    localStorage.removeItem("courseref");
    localStorage.setItem("courseref", JSON.stringify(data));
    // toast.success("course added to local storage!");
  };
      ...
      <div className="pt-5">
        {chunkedCourses.map((chunk, rowIndex) => (
          <div key={rowIndex} className="d-flex justify-content-center">
            {chunk.map((course) => (
              matchingCourseNamesData===course.coursename ?
              <Col sm={4} key={course.id}>
                <Link
                  to={`/course/${course.id}`}
                  className="text-decoration-none"
                  onClick={() => {
                    clearAndAddToLocalStorage(course);
                  }}
                >
                  <Card
